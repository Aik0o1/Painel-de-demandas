"""
Conversor XLSX -> dict (relatorio de produtividade JUCEPI).

Le a planilha mensal ("<MES>-ANALISE", "ANALISE GERAL", "TEMPO ANALISTAS - <MES>",
"MEDIA TEMPO GERAL", "<MES> - CALL CENTER", "EMPRESAS ATIVAS") e retorna um dicionario
no mesmo formato usado pelos documentos do CouchDB / `pdf_generator.py`.

Os blocos de dados sao localizados pelo texto dos cabecalhos (ancoras), nao por indices
fixos de linha/coluna, pois cada planilha mensal tem um numero variavel de linhas por
bloco (mais ou menos analistas, atendentes etc.).
"""
...
class ExcelReportError(ValueError):
    """Erro de leitura/validacao da planilha de produtividade."""
...
def resolve_sheets(sheets):
    ...
    if not analise_name:
        raise ExcelReportError("Não encontrei a aba principal '<MÊS>-ANALISE' na planilha.")
    ...
    if missing:
        raise ExcelReportError(f"Abas obrigatórias não encontradas: {', '.join(missing)}")
...
def converter_para_dict(source) -> dict:
    """Converte uma planilha de produtividade em um dicionario.

    `source` pode ser um caminho de arquivo, bytes ou qualquer objeto aceito por
    `pandas.read_excel` (ex: BytesIO de um upload).
    """
    try:
        sheets = pd.read_excel(source, sheet_name=None, header=None)
    except Exception as e:
        raise ExcelReportError(f"Não foi possível ler o arquivo como planilha Excel: {e}") from e
